...
from calculate import calculate_cost
    ...
    def __init__(self, master):
        ...
        with open('purchase_requests.csv', 'a', newline='') as file:
            ...
            if file.tell() == 0:  # 如果檔案是空的，寫入表頭
                writer.writerow(['申請人 applicant','部門 Department', '購買物品 Item', '單項費用 UnitPrice', '數量 Quantity', '用途 Purpose', '總金額 TotalCost','品項名稱Product Item Detailed Name'])
        ...
        self.note_label.grid(row=7, column=0, padx=10, pady=10, sticky="nw")
        self.note_entry = tk.Text(master, height=5, width=30)
        self.note_entry.grid(row=7, column=1, padx=10, pady=10)
...
def submit_request(self):
    ...
    cost_calculator = calculate_cost(float(unit_price), int(quantity))
    total_cost = cost_calculator.total_cost()
    ...
    with open('purchase_requests.csv', 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([name, department, item, unit_price, quantity, purpose, total_cost, note])
